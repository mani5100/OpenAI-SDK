...
10. **Create Jira Tasks:** Using the Atlassian MCP integration, automatically create tasks in Jira:
    - Create an Epic or Parent Story for the feature (using the PRD title)
    - For each parent task (e.g., "1.0 Parent Task Title"), create a Jira Story linked to the Epic
    - For each sub-task (e.g., "1.1 Sub-task description"), create a Jira Sub-task under its corresponding parent Story
    - Include relevant details in the Jira task description (link to PRD, acceptance criteria, relevant files)
    - Maintain the same numbering/hierarchy structure in Jira task titles for easy tracking
    - Confirm with the user after tasks are created, providing Jira task IDs/links
